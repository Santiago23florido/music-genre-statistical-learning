...
For the implementation of the unsupervised classifier, Ward's hierarchical clustering is applied to the PCA scores. Several reduced representations are considered by retaining \(d \in \{4,6,10,34,165\}\) principal components. The choice \(d=4\) corresponds to the first two principal planes. The values \(d=6\) and \(d=10\) are included because the scree plot of the percentage of inertia explained by each component shows relevant breaks before regions where the marginal gains in inertia become more homogeneous. Finally, \(d=34\) is considered because it corresponds to the set of principal components whose individual contribution remains above the average contribution expected under a uniform distribution of inertia across the variables and \(d=165\) corresponds to all of the components.
