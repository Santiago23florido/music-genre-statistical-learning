...
Based on the empirical evaluation on the test set, \textbf{ModAIC} achieves the highest AUC (\(0.9629\)) and the second highest accuracy (\(0.9082\)). It also provides a more parsimonious representation than the full and ridge models by reducing the active predictor space from \(165\) to \(91\) variables. Consequently, \textbf{ModAIC} is selected as the final binary classifier for this task. The genre predictions for the unlabeled holdout file were generated using this model and saved to the corresponding submission file. Additional classification models, including Lasso and Relaxed Lasso, were also explored (see Appendix~\ref{app:bonus_lasso}), but none improved the test AUC obtained by ModAIC under this criterion.
